jQuery( document ).ready(function () {
	...
	function coauthors_add_coauthor( author, co, init, count ){
		...
		if ( co && co.siblings( '.coauthor-tag' ).length ) {
			...
			co.parent().append('<input class="coauthors_affiliations" placeholder="Author affiliation" type="text" name="coauthors_affiliations[]" value="' + author.affiliation + '" />');
	...
	function coauthors_autosuggest_select() {
		...
		author.affiliation = jQuery.trim( vals[5] );
	...
	if ( 'post-php' == adminpage || 'post-new-php' == adminpage ) {
		...
		var $post_coauthor_affiliations = jQuery( 'input[name="coauthorsaffiliations[]"]' );
		...
		for ( var i = 0; i < $post_coauthor_logins.length; i++ ) {
			post_coauthors.push({
				...
				affiliation: $post_coauthor_affiliations[i].value
			});
		}
